...
class Downloader {
  ...
  public static function downloadRemote(): void {
    ...
    Output::status(sprintf('Downloading DrevOps from the remote repository "%s" at ref "%s".', $url, $ref), Output::STATUS_MESSAGE, FALSE);
    ...
    Output::status(sprintf('Downloaded to "%s".', $dst), Output::STATUS_DEBUG);

    Output::status('Done', Output::STATUS_SUCCESS);
  }

  public static function downloadLocal(): void {
    $dst = Config::getInstance()->get(Env::INSTALLER_TMP_DIR);
    $repo = Config::getInstance()->get(Env::INSTALLER_LOCAL_REPO);
    $ref = Config::getInstance()->get(Env::INSTALLER_COMMIT);

    Output::status(sprintf('Downloading DrevOps from the local repository "%s" at ref "%s".', $repo, $ref), Output::STATUS_MESSAGE, FALSE);
    ...
    Output::status(implode(PHP_EOL, $output), Output::STATUS_DEBUG);
  ...
  public function download(): void {
    if (Config::getInstance()->get(Env::INSTALLER_LOCAL_REPO)) {
