...
class InstallManager {
  ...
  public function prepareDestination(): void {
    ...
    if (!is_dir($dst)) {
      Output::status(sprintf('Creating destination directory "%s".', $dst), Output::STATUS_MESSAGE, FALSE);
      mkdir($dst);
      if (!is_writable($dst)) {
    ...
    if (is_readable($dst . '/.git')) {
      Output::status(sprintf('Git repository exists in "%s" - skipping initialisation.', $dst), Output::STATUS_MESSAGE, FALSE);
    }
    else {
      Output::status(sprintf('Initialising Git repository in directory "%s".', $dst), Output::STATUS_MESSAGE, FALSE);
  ...
  public function copyFiles(): void {
    ...
    Output::status('Copying files', Output::STATUS_DEBUG);
    ...
    foreach ($valid_files as $filename) {
      ...
      if (Files::isInternalPath($relative_file)) {
        Output::status(sprintf('Skipped file %s as an internal DrevOps file.', $relative_file), Output::STATUS_DEBUG);
        unlink($filename);
        continue;
      }
    }
