    ...
    $this->promptManager->askQuestions(function (PromptManager $qm): void {
      ...
      if ($qm->getAnswer(ProvisionUseProfilePrompt::ID)) {
        ...
        if ($qm->getAnswer(DatabaseDownloadSourcePrompt::ID) != DatabaseDownloadSourcePrompt::CHOICE_DOCKER_REGISTRY) {
          // Note that "database_store_type" is a pseudo-answer - it is only
          // used to improve UX and is not exposed as a variable (although, it
          // has default, discovery and normalisation callbacks).
